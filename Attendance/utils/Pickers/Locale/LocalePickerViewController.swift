...
extension UIAlertController {
  ...
  func addLocalePicker(type: LocalePickerViewController.Kind, selection: @escaping LocalePickerViewController.Selection) {
    ...
    let vc = LocalePickerViewController(type: type) { new in
      ...
      let buttonCancel: UIAlertAction = UIAlertAction(title: "Cancel", style: .default) { _ in
          self.dismiss(animated: true)
      }
    set(vc: vc)
    addAction(buttonSelect)
    addAction(buttonCancel)
  }
...
//          controller.searchBar.textField?.textColor = .black
//          controller.searchBar.textField?.clearButtonMode = .whileEditing
          return controller
    }()
  ...
  override func viewWillLayoutSubviews() {
    ...
    tableView.tableHeaderView?.frame.size.height = 57.0
    ...
    LocaleStore.fetch { [unowned self] result in
      ...
      case .error(let error):
        ...
        DispatchQueue.main.async {
          ...
          //                  let action = UIAlertAction.init(title: "OK", style: .cancel, handler: { (action: UIAlertAction) in
          //
          //                    self.indicatorView.stopAnimating()
...
extension LocalePickerViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    DispatchQueue.main.async {
      let _: CGSize = CGSize(width: 32, height: 24)
      let flag: UIImage? = info.flag
